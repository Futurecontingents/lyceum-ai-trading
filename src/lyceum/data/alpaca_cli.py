...
class AlpacaCliGateway:
    ...
    def _json(self, *args: str) -> dict[str, Any]:
        completed = subprocess.run(["alpaca", "--profile", self.profile, *args], capture_output=True, text=True, timeout=self.timeout, check=False)
    ...
    def account(self) -> PortfolioState:
        ...
        return PortfolioState(equity=float(data["equity"]), buying_power=float(data["buying_power"]), daily_realized_pnl=0.0, open_positions=0, open_risk=0.0)
    ...
    def bars(self, symbol: str, *, timeframe: str = "1Hour", days: int = 45, limit: int = 1000) -> list[dict[str, Any]]:
        ...
        data = self._json("data", "bars", "--symbol", symbol, "--start", start, "--timeframe", timeframe, "--feed", "iex", "--limit", str(limit))
    ...
    def market_snapshot(self, symbol: str) -> MarketSnapshot:
        ...
        return MarketSnapshot(symbol=symbol, timestamp=datetime.fromisoformat(str(bars[-1]["t"]).replace("Z", "+00:00")), price=closes[-1], previous_close=closes[-2], momentum_1h=returns[-1], momentum_1d=closes[-1] / closes[max(0, len(closes) - 7)] - 1, realized_volatility=max(0.01, realized))

    def option_chain(self, symbol: str, price: float, *, days_min: int = 7, days_max: int = 35, limit: int = 100) -> list[OptionContract]:
        today = datetime.now(timezone.utc).date()
        data = self._json("data", "option", "chain", "--underlying-symbol", symbol, "--expiration-date-gte", (today + timedelta(days=days_min)).isoformat(), "--expiration-date-lte", (today + timedelta(days=days_max)).isoformat(), "--strike-price-gte", f"{price * 0.9:.2f}", "--strike-price-lte", f"{price * 1.1:.2f}", "--limit", str(limit))
        ...
        for option_symbol, snapshot in data.get("snapshots", {}).items():
            ...
            if not match or not quote.get("t"):
                continue
            contracts.append(OptionContract(symbol=option_symbol, underlying=symbol, expiration=datetime.strptime(match.group("date"), "%y%m%d").date().isoformat(), strike=int(match.group("strike")) / 1000, option_type="call" if match.group("type") == "C" else "put", bid=float(quote.get("bp") or 0), ask=float(quote.get("ap") or 0), bid_size=int(quote.get("bs") or 0), ask_size=int(quote.get("as") or 0), implied_volatility=snapshot.get("impliedVolatility"), delta=(snapshot.get("greeks") or {}).get("delta"), quote_timestamp=datetime.fromisoformat(str(quote["t"]).replace("Z", "+00:00"))))
        return contracts
